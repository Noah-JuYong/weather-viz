...
수집 단계에서 API 요청이 실패하면 기존 CSV를 덮어쓰지 않고 실행을 실패시킨다. 분석과
렌더링 단계는 필요한 CSV 열이나 템플릿이 없으면 실패하므로, 불완전한 데이터를 리포트로
배포하지 않는다. 배포 단계는 생성한 `site/` 전체를 Pages 아티팩트로 업로드하며, 업로드나
배포에 실패하면 이전 Pages 배포본이 유지된다.
